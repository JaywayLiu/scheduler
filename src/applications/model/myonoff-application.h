...
public:
   ...
   */
  Ptr<Socket> GetLteSocket (void) const;
  Ptr<Socket> GetWifiSocket (void) const;
 /**
...
private:
  ...
  int             m_current_on;       //0 lte 1 wifi
  Ptr<Socket>     m_lte_socket;       //!< Associated socket
  Ptr<Socket>     m_wifi_socket;       //!< Associated socket
  Ptr<Socket>     m_current_socket;       //!< Associated socket
  Address         m_lte_peer;         //!< Peer address
  Address         m_wifi_peer;         //!< Peer address
  Address         m_current_peer;         //!< Peer address
  bool            m_lte_connected;    //!< True if connected
  bool            m_wifi_connected;    //!< True if connected
  //Ptr<RandomVariableStream>  m_onTime;       //!< rng for On Time
  //Ptr<RandomVariableStream>  m_offTime;      //!< rng for Off Time
  DataRate        m_cbrRate;      //!< Rate that data is generated
  DataRate        m_cbrRateFailSafe;      //!< Rate that data is generated (check copy)
  uint32_t        m_pktSize;      //!< Size of packets
  uint32_t        m_lte_residualBits; //!< Number of generated, but not sent, bits on lte
  uint32_t        m_wifi_residualBits; //!< Number of generated, but not sent, bits on wifi
