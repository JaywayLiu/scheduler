...
OpenFlowSwitchNetDevice::BufferFromPacket (Ptr<Packet> packet, Address src, Address dst, int mtu, uint16_t protocol)
{
  ...
  //Ptr<Packet> newpacket = CreateObject<Packet>(&packet);
          ...
          if (packet->PeekHeader (udp_hd))
            {
              ...
              //std::cout<<udp_hd.GetSourcePort ()<<" "<<udp_hd.GetDestinationPort ()<<std::endl;
  ...
  if (buffer->l2)
   ...
   //add ip/arp header back
   if (protocol == Ipv4L3Protocol::PROT_NUMBER)
    {
      packet->AddHeader(ip_hd);
    }
   else
    {
      packet->AddHeader(arp_hd);
    }
  return buffer;
}
